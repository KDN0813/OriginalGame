...
void GamePad::Update()
{
	...
	// キーボードでエミュレーション
	{
		...
		if (GetAsyncKeyState(VK_SPACE) & 0x8000) new_button_state |= BTN_A;
...
#ifdef _DEBUG
		if (GetAsyncKeyState(VK_LSHIFT) & 0x8000 && GetAsyncKeyState(VK_F1) & 0x8000)	new_button_state |= BTN_IMGUI;
